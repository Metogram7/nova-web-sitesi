...
for idx, key in enumerate(GEMINI_API_KEYS):
    ...
    print(f"[KEYS] Key #{idx}: {masked} (Uzunluk: {len(key)})")
print(f"[OK] Toplam {len(GEMINI_API_KEYS)} Gemini key yuklendi.")
...
async def gemma_cevap_async(message, conversation, sess, user_name=None, image_data=None, custom_prompt=""):
    ...
    # Normalize contents for Gemini API (Must alternate user/model and start with user)
    normalized_contents = []
    for c in contents:
        ...
        if not normalized_contents:
            if c["role"] == "model":
                normalized_contents.append({"role": "user", "parts": [{"text": "[Bağlam Başlangıcı]"}]})
            normalized_contents.append(c)
        else:
            if normalized_contents[-1]["role"] == c["role"]:
                if "text" in normalized_contents[-1]["parts"][0] and "text" in c["parts"][0]:
                    normalized_contents[-1]["parts"][0]["text"] += "\n\n" + c["parts"][0]["text"]
                else:
                    normalized_contents[-1]["parts"].extend(c["parts"])
            else:
                normalized_contents.append(c)

    # Gemini API cannot have model as the last message (though we appended a user msg above)
    if normalized_contents and normalized_contents[-1]["role"] == "model":
        normalized_contents.append({"role": "user", "parts": [{"text": "Devam et."}]})
...
async def ws_chat_handler():
    ...
    while True:
        try:
            ...
            # Save history only if we got a valid response (not starting with [!] error prefix)
